...
jest.mock('../../../../lib/platforms', () => ({
  PlatformManager: jest.fn().mockImplementation(() => ({
    detect: jest.fn().mockResolvedValue({
      name: 'GitHub',
      createPullRequest: jest.fn().mockResolvedValue({ number: 123, url: 'https://github.com/pr/123' }),
      createIssue: jest.fn().mockResolvedValue({ number: 456, url: 'https://github.com/issue/456' }),
...
describe('GitHub Actions', () => {
  ...
  test('should warn when not in GitHub repository', async () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('Not in a GitHub repository'));
  });
  ...
  test('should create pull request', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Pull request created'));
  });
  ...
  test('should create issue', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Issue created'));
  });
  ...
  test('should update pull request', async () => {
    ...
    expect(actions.platform.updatePullRequest).toHaveBeenCalledWith(123, { title: 'Updated Title' });
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Pull request #123 updated'));
  });
  ...
  test('should update issue', async () => {
    ...
    expect(actions.platform.updateIssue).toHaveBeenCalledWith(456, { labels: ['enhancement'] });
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Issue #456 updated'));
  });
  ...
  test('should create label', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Label "documentation" created'));
  });
  ...
  test('should handle errors gracefully', async () => {
    await actions.initialize();
    actions.platform.createPullRequest.mockRejectedValue(new Error('API error'));
    
    await expect(actions.createPR({})).rejects.toThrow('API error');
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Failed to create pull request'));
  });
});