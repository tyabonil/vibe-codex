...
describe('Validate Command', () => {
  ...
  test('should validate project with default options', async () => {
    ...
    expect(mockSpinner.start).toHaveBeenCalledWith('Loading vibe-codex configuration...');
  ...
  test('should handle missing configuration', async () => {
    ...
    await expect(validate({})).rejects.toThrow('No vibe-codex configuration found');
  });
  ...
  test('should filter by specific modules', async () => {
    ...
    expect(mockSpinner.info).toHaveBeenCalledWith('Validating modules: core, testing');
  });

  test('should output JSON format', async () => {
    await validate({ json: true });
    
    const output = console.log.mock.calls.find(call => 
  ...
  test('should show verbose output', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Validation Details'));
  });
  ...
  test('should attempt to fix violations', async () => {
    ...
    expect(mockSpinner.info).toHaveBeenCalledWith('Attempting to auto-fix violations...');
  });

  test('should handle validation errors', async () => {
    const validator = require('../../../lib/validator');
    validator.validateProject.mockRejectedValueOnce(new Error('Validation failed'));
  ...
  test('should run hook-specific validation', async () => {
    ...
    expect(mockSpinner.info).toHaveBeenCalledWith('Running pre-commit hook validation');
  });
