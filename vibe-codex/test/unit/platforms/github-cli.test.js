...
describe('GitHubCLI', () => {
  ...
  describe('isAvailable', () => {
    test('should return true when gh CLI is available', async () => {
      mockExecAsync.mockResolvedValue({ stdout: 'gh version 2.0.0', stderr: '' });
  ...
  describe('execute', () => {
    test('should execute command and parse JSON output', async () => {
      ...
      expect(mockExecAsync).toHaveBeenCalledWith('gh pr view 123', expect.any(Object));
    });
    ...
    test('should throw error on command failure', async () => {
      ...
      await expect(cli.execute('pr', ['create'])).rejects.toThrow('GitHub CLI error: Command failed');
    });
  });
  ...
  describe('getBranches', () => {
    test('should get list of branches', async () => {
      ...
      expect(mockExecAsync).toHaveBeenCalledWith('gh api repos/:owner/:repo/branches --jq .[].name', expect.any(Object));
    });
  });
