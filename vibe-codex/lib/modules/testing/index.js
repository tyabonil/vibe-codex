...
export class TestingModule extends RuleModule {
  ...
  async loadRules() {
    ...
    this.registerRule({
      ...
      severity: 'error',
      check: async (context) => {
        const threshold = context.config?.testing?.coverageThreshold || this.options.coverageThreshold;
        
        try {
          // Try to read coverage report
          const coverageReportPath = path.join(context.projectPath, 'coverage', 'coverage-summary.json');
          const coverageData = JSON.parse(await fs.readFile(coverageReportPath, 'utf8'));
      ...
      check: async (context) => {
        ...
        for (const sourceFile of sourceFiles) {
          const testPatterns = [
            ...
            sourceFile.path.replace('/src/', '/__tests__/').replace(/\.(js|ts|jsx|tsx)$/, '.test.$1'),
            sourceFile.path.replace('/src/', '/test/').replace(/\.(js|ts|jsx|tsx)$/, '.test.$1')
          ];
          
          const hasTest = await Promise.all(
            testPatterns.map(pattern => 
              fs.access(pattern).then(() => true).catch(() => false)
            )
          );
      ...
      check: async (context) => {
        ...
        for (const file of testFiles) {
          ...
          while ((match = describePattern.exec(content)) !== null) {
            descriptions.push({ type: 'describe', text: match[1], line: content.substring(0, match.index).split('\n').length });
          }
          
          while ((match = itPattern.exec(content)) !== null) {
            descriptions.push({ type: 'it', text: match[1], line: content.substring(0, match.index).split('\n').length });
          }
          
          while ((match = testPattern.exec(content)) !== null) {
            descriptions.push({ type: 'test', text: match[1], line: content.substring(0, match.index).split('\n').length });
          }
          
          for (const desc of descriptions) {
            // Check for poor descriptions
            if (desc.text.length < 5 || desc.text === 'test' || desc.text === 'works') {
              violations.push({
                file: file.path,
                line: desc.line,
            ...
            if (desc.type === 'it' && !desc.text.match(/^(should|returns|throws|handles|validates|creates|updates|deletes)/i)) {
              violations.push({
                file: file.path,
                line: desc.line,
      ...
      check: async (context) => {
        ...
        for (const file of testFiles) {
          ...
          lines.forEach((line, index) => {
            ...
            if (line.match(/\.(skip|only)\s*\(/) || line.match(/(describe|it|test)\.skip/)) {
              violations.push({
                file: file.path,
                line: index + 1,
    ...
    this.registerRule({
      ...
      check: async (context) => {
        ...
        for (const file of testFiles) {
          ...
          const hasAsyncOperations = content.includes('async') || content.includes('await');
          const hasSetup = content.match(/(beforeEach|beforeAll|afterEach|afterAll)/);
          
          if (hasAsyncOperations && !hasSetup) {
            violations.push({
              file: file.path,
              message: 'Consider using setup/teardown hooks for async operations'
            });
          }
        }
  ...
  async loadHooks() {
    ...
    this.registerHook('pre-push', async (context) => {
      ...
      try {
        ...
        const { stdout, stderr } = await execAsync(testCommand, { cwd: context.projectPath });
    ...
    this.registerValidator('test-framework', async (projectPath) => {
      try {
        ...
        const hasTestFramework = testFrameworks.some(framework => 
          packageJson.devDependencies?.[framework] || packageJson.dependencies?.[framework]
        );
