...
export class GitHubModule extends RuleModule {
  ...
  async loadRules() {
    ...
    this.registerRule({
      ...
      check: async (context) => {
        ...
        const hasTemplate = await Promise.all(
          prTemplates.map(template => 
            fs.access(path.join(context.projectPath, template))
              .then(() => true)
              .catch(() => false)
          )
        );
        ...
        if (!hasTemplate.some(exists => exists)) {
          return [{
            message: 'No pull request template found. Create .github/pull_request_template.md'
          }];
        }
      ...
      fix: async (context) => {
        const templatePath = path.join(context.projectPath, '.github', 'pull_request_template.md');
...
`;
    ...
    this.registerRule({
      ...
      check: async (context) => {
        ...
        const issueTemplateDir = path.join(context.projectPath, '.github', 'ISSUE_TEMPLATE');
        
        try {
          const files = await fs.readdir(issueTemplateDir);
          const templates = files.filter(f => f.endsWith('.md') || f.endsWith('.yml'));
          
          if (templates.length === 0) {
            return [{
              message: 'Issue template directory exists but contains no templates'
            }];
          }
          
          // Check for basic template types
          const hasFeatureRequest = templates.some(t => t.toLowerCase().includes('feature'));
          const hasBugReport = templates.some(t => t.toLowerCase().includes('bug'));
          ...
          return [{
            message: 'No issue templates found. Create .github/ISSUE_TEMPLATE/ directory with templates'
          }];
        }
      },
      fix: async (context) => {
        const templateDir = path.join(context.projectPath, '.github', 'ISSUE_TEMPLATE');
...
`;
        ...
        await fs.writeFile(path.join(templateDir, 'bug_report.md'), bugTemplate);
        await fs.writeFile(path.join(templateDir, 'feature_request.md'), featureTemplate);
        return true;
      }
    });
      ...
      check: async (context) => {
        ...
        const hasCodeowners = await Promise.all(
          codeownersLocations.map(location => 
            fs.access(path.join(context.projectPath, location))
              .then(() => true)
              .catch(() => false)
          )
        );
        ...
        if (!hasCodeowners.some(exists => exists)) {
          return [{
            message: 'No CODEOWNERS file found for automatic review assignments'
          }];
        }
    ...
    this.registerRule({
      ...
      check: async (context) => {
        ...
        try {
          ...
          const hasTests = workflowContent.some(w => 
            w.content.includes('test') || w.content.includes('jest') || w.content.includes('mocha')
          );
          const hasLint = workflowContent.some(w => 
            w.content.includes('lint') || w.content.includes('eslint')
          );
