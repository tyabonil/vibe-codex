...
export class DeploymentModule extends RuleModule {
  ...
  async loadRules() {
    ...
    this.registerRule({
      ...
      check: async (context) => {
        const violations = [];
        const platforms = context.config?.deployment?.platforms || this.options.platforms;
          ...
          if (platformConfigs[platform]) {
            const hasConfig = await Promise.all(
              platformConfigs[platform].map(config => 
                fs.access(path.join(context.projectPath, config))
                  .then(() => true)
                  .catch(() => false)
              )
            );
            ...
            if (hasConfig.some(exists => exists)) {
              ...
              const configViolations = await this.validatePlatformConfig(context, platform);
      ...
      check: async (context) => {
        ...
        try {
          ...
          const hasBuildDir = await Promise.all(
            buildDirs.map(dir => 
              fs.access(path.join(context.projectPath, dir))
                .then(() => true)
                .catch(() => false)
            )
          );
          
          if (!hasBuildDir.some(exists => exists) && packageJson.scripts?.build) {
            // Check if build directory is in .gitignore
            try {
              const gitignore = await fs.readFile(path.join(context.projectPath, '.gitignore'), 'utf8');
              const hasBuildDirIgnored = buildDirs.some(dir => gitignore.includes(dir));
      ...
      check: async (context) => {
        ...
        try {
          ...
          if (!hasMultiStage && lines.length > 20) {
            violations.push({
              message: 'Consider using multi-stage Docker builds for smaller images'
            });
          }
          ...
          const copyPackageIndex = lines.findIndex(line => 
            line.includes('COPY package*.json') || line.includes('COPY package.json')
          );
          const copyAllIndex = lines.findIndex(line => 
            line.includes('COPY . .') || line.includes('COPY ./')
          );
          
          if (copyPackageIndex > copyAllIndex && copyPackageIndex !== -1 && copyAllIndex !== -1) {
            violations.push({
              message: 'Copy package.json before source code for better Docker cache utilization'
            });
          }
        } catch (error) {
    ...
    this.registerRule({
      ...
      check: async (context) => {
        ...
        if (!hasHealthCheck) {
          ...
          const hasExpress = sourceFiles.some(f => f.content.includes('express()'));
          const hasFastify = sourceFiles.some(f => f.content.includes('fastify('));
          const hasKoa = sourceFiles.some(f => f.content.includes('new Koa()'));
          
          if (hasExpress || hasFastify || hasKoa) {
            violations.push({
              message: 'No health check endpoint found. Add /health or /status endpoint'
            });
          }
        }
      ...
      case 'vercel':
        try {
          const vercelConfig = JSON.parse(
            await fs.readFile(path.join(context.projectPath, 'vercel.json'), 'utf8')
          );
