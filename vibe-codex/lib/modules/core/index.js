...
export class CoreModule extends RuleModule {
  ...
  async loadRules() {
    ...
    this.registerRule({
      ...
      check: async (context) => {
        const hasEnvFile = context.files.some(f => 
          f.path.endsWith('.env') || f.path.endsWith('.env.local')
        );
    ...
    this.registerRule({
      ...
      description: 'Branch must reference issue: feature/issue-{number}-{description}',
      level: 2,
      category: 'workflow',
      severity: 'error',
      check: async (context) => {
        ...
        if (!branchPattern.test(context.branch)) {
          return [{
            ...
            message: 'Branch name must follow pattern: {type}/issue-{number}-{description}'
          }];
        }
        return [];
      }
