...
class GitHubPlatform extends GitPlatform {
  ...
  async initialize() {
    ...
    for (const preferred of this.preferredMethods) {
      if (this.methods.includes(preferred)) {
        ...
        console.log(chalk.blue(`Using GitHub ${preferred.toUpperCase()} method`));
        break;
      }
    }
    ...
    if (!this.activeMethod) {
      ...
      console.log(chalk.yellow(`Using fallback GitHub ${firstMethod.toUpperCase()} method`));
    }
  }
  ...
  async executeWithFallback(operation, ...args) {
    ...
    for (const [name, method] of Object.entries(this.availableMethods)) {
      ...
      try {
        console.log(chalk.yellow(`Falling back to GitHub ${name.toUpperCase()} method`));
    ...
    // All methods failed
    const errorMessages = errors.map(e => `${e.method}: ${e.error.message}`).join('\n');
  ...
  async updatePullRequest(prNumber, updates) {
    return await this.executeWithFallback('updatePullRequest', prNumber, updates);
  }
