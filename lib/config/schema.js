...
const configSchema = Joi.object({
  ...
  project: Joi.object({
    ...
    type: Joi.string().valid('web', 'api', 'fullstack', 'library', 'custom').default('custom'),
    description: Joi.string().optional()
  }).optional(),
  
  // Preset used (if any)
  preset: Joi.string().valid('minimal', 'standard', 'strict', 'ai-developer', 'enterprise', 'custom').default('custom'),
...
function migrateV2ToV3(v2Config) {
  ...
  if (Object.keys(v3Config.rules).length === 0) {
    ...
    const standardRules = ['sec-001', 'sec-002', 'sec-003', 'cmt-001', 'tst-003', 'doc-001', 'sty-001', 'sty-002', 'wfl-006'];
