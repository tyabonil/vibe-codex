...
function box(options = {}) {
  ...
  const maxContentLength = Math.max(...lines.map(line => stripAnsi(line).length));
  ...
  if (title) {
    ...
    result += colorize(border.topLeft + border.horizontal.repeat(titlePadding) + ' ');
    result += chalk.bold(title);
    result += colorize(' ' + border.horizontal.repeat(boxWidth - titlePadding - titleLength - 2) + border.topRight);
  } else {
    result += colorize(border.topLeft + border.horizontal.repeat(boxWidth - 2) + border.topRight);
  }
  result += '\n';
  
  // Add padding lines
  for (let i = 0; i < padding; i++) {
    result += colorize(border.vertical) + ' '.repeat(boxWidth - 2) + colorize(border.vertical) + '\n';
  }
  ...
  lines.forEach(line => {
    ...
    result += colorize(border.vertical) + ' '.repeat(leftPadding) + line + ' '.repeat(rightPadding) + colorize(border.vertical) + '\n';
  });
  ...
  // Build bottom border
  result += colorize(border.bottomLeft + border.horizontal.repeat(boxWidth - 2) + border.bottomRight);
...
function header(text, options = {}) {
  ...
  return chalk[color]([
    line,
    '║' + ' '.repeat(padding) + chalk.bold(text) + ' '.repeat(width - padding - text.length - 2) + '║',
    line
  ].join('\n'));
}
