      ...
      check: async (context) => {
        ...
        try {
          ...
          const requiredSections =
            context.config?.documentation?.requiredReadmeSections ||
            this.options.requiredReadmeSections;
          const missingSections = [];

          for (const section of requiredSections) {
            // Escape special regex characters in section name
            const escapedSection = section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const sectionRegex = new RegExp(`#+\\s*${escapedSection}`, "i");
          ...
          if (!content.includes("```") && lowerContent.includes("usage")) {
            violations.push({
              message:
                "README.md should include code examples in usage section",
            });
          }
        ...
        if (!hasApiDocs.some((exists) => exists)) {
          ...
          if (filesWithoutDocs.length > 0) {
            return [
              {
                message:
                  "No API documentation found. Create API.md or add JSDoc comments",
      ...
      check: async (context) => {
        ...
        try {
          ...
          if (!hasCategories) {
            violations.push({
              message:
                "CHANGELOG.md should use standard categories (Added, Changed, etc.)",
            });
          }
          ...
          const daysSinceUpdate =
            (lastCommitDate - changelogDate) / (1000 * 60 * 60 * 24);
...
`;
  ...
  async loadHooks() {
    ...
    this.registerHook("pre-commit", async (context) => {
      ...
      const readmeModified = modifiedFiles.some((f) =>
      ...
      const hasCodeChanges = modifiedFiles.some(
        (f) =>
          f.endsWith(".js") ||
          f.endsWith(".ts") ||
          f.endsWith(".jsx") ||
      ...
      const changelogModified = modifiedFiles.some((f) =>
  ...
  async loadValidators() {
    ...
    this.registerValidator("doc-tools", async (projectPath) => {
      try {
        ...
        const hasDocTool = docTools.some(
          (tool) =>
