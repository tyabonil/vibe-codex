...
module.exports = async function validate(options) {
  ...
  const task = simpleLogger.startTask("Loading configuration");
  ...
  try {
    ...
    if (!(await fs.pathExists(configPath))) {
      ...
      simpleLogger.warning('Run "npx vibe-codex init" to set up vibe-codex');
      return;
    }

    // const config = await fs.readJSON(configPath);
    task.succeed("Configuration loaded");

    simpleLogger.section("Running vibe-codex validation");

    // Initialize module loader
    const moduleTask = simpleLogger.startTask("Loading rule modules");
    ...
    const validationTask = simpleLogger.startTask("Checking rules compliance");
    const results = await runModularValidation();
    validationTask.succeed("Validation complete");
    ...
    simpleLogger.error(`Validation failed: ${error.message}`);
    throw error;
  }
};
 ...
 */
async function runModularValidation() {
  const results = {
    violations: [],
    warnings: [],
    passed: [],
