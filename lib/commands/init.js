...
module.exports = async function init(options) {
  ...
  try {
    ...
    if (options.withAdvancedHooks) {
      const categories = options.withAdvancedHooks.split(',').map(c => c.trim()).filter(c => c);
      if (categories.length > 0) {
        advancedHooksConfig = {
          enabled: true,
          categories: categories
        };
        logger.output(chalk.green(`✓ Will install advanced hooks: ${categories.join(', ')}`));
      }
    }
    // Skip advanced hooks in minimal mode
    else if (!options.minimal) {
      const { installAdvancedHooks } = await inquirer.prompt([
        {
          type: "confirm",
          name: "installAdvancedHooks",
          message: "Install advanced development hooks? (PR health checks, issue tracking, etc.)",
          default: false,
        },
      ]);

      if (installAdvancedHooks) {
        const { advancedHooks } = require('../config/advanced-hooks');
        const hookChoices = Object.entries(advancedHooks).map(([key, value]) => ({
    ...
    try {
      ...
      // Validation command sets process.exitCode, not throws
      spinner.warn("Initial validation completed with warnings");
    }

    // Show success message and next steps
    showSuccessMessage(config, selectedModules, setupValidation.packageManager, advancedHooksConfig);
  } catch (error) {
      ...
      choices: [
        {
          ...
          value: "testing",
          checked: defaultModules.includes("testing"),
        },
        {
          name: "GitHub Workflow Integration",
          value: "github",
          checked: defaultModules.includes("github"),
        },
        {
          name: "Deployment Validation",
          value: "deployment",
          checked: defaultModules.includes("deployment"),
        },
        {
          name: "Documentation Requirements",
          value: "documentation",
          checked: defaultModules.includes("documentation"),
        },
        {
          name: "Code Quality (ESLint/Prettier)",
          value: "quality",
          checked: defaultModules.includes("quality"),
        },
      ],
    },
  ]);
...
function showSuccessMessage(config, selectedModules, packageManager = "npm", advancedHooksConfig = null) {
