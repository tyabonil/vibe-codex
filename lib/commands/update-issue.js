...
async function main() {
  ...
  if (args.length < 1) {
    logger.error('Usage: npx vibe-codex update-issue <issue-number> [type] [message]');
    ...
    case 'plan':
      if (!message) {
        ...
        await updater.postPlan({
          ...
          tasks: planAnswers.tasks.split(',').map(t => t.trim()).filter(Boolean)
        });
      } else {
    ...
    case 'blocker':
      if (!message) {
        ...
        await updater.postBlocker({
          ...
          attempted: blockerAnswers.attempted.split(',').map(t => t.trim()).filter(Boolean),
    ...
    case 'completion':
      if (!message) {
        ...
        await updater.postCompletion({
          ...
          implemented: completionAnswers.implemented.split(',').map(t => t.trim()).filter(Boolean),
          tested: completionAnswers.tested ? 'All tests written and passing' : 'Tests pending',
