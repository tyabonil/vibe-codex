...
const RULES_URL = 'https://raw.githubusercontent.com/tyabonil/cursor_rules/main/MANDATORY-RULES.md';
const PROJECT_CONTEXT_URL = 'https://raw.githubusercontent.com/tyabonil/cursor_rules/main/PROJECT_CONTEXT.md';
...
- Modules: ${config.modules ? Object.keys(config.modules).filter(m => config.modules[m].enabled).join(', ') : 'core'}
...
async function createConfigFiles(config) {
  ...
  await fs.writeJSON('config/project-patterns.json', projectPatterns, { spaces: 2 });
  
  // Create rules.json with project-specific overrides
  const rules = {
    "version": "1.0.0",
    "projectType": config.projectType,
    "enabledModules": Object.keys(config.modules || {}).filter(m => config.modules[m].enabled),
    "customRules": []
  };
