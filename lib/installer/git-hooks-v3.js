...
`;
  ...
  // Dependency safety check (wfl-009)
  if (rules.some(r => r.ruleId === 'wfl-009')) {
    script += `
# Dependency safety check for deleted files
DELETED_FILES=$(git diff --cached --name-only --diff-filter=D)
if [ -n "$DELETED_FILES" ]; then
  echo "🔍 Checking dependencies for deleted files..."
  if [ -f "${projectRoot}/templates/hooks/dependency-check-hook.sh" ]; then
    echo "$DELETED_FILES" | while read -r file; do
      if [ -n "$file" ]; then
        bash "${projectRoot}/templates/hooks/dependency-check-hook.sh" "$file" || FAILED=1
      fi
    done
  fi
fi
`;
  ...
  // Goal alignment verification (ai-005)
  if (rules.some(r => r.ruleId === 'ai-005')) {
    script += `
# Goal alignment check for AI agents
if [ -f "${projectRoot}/templates/hooks/goal-alignment-check.sh" ]; then
  bash "${projectRoot}/templates/hooks/goal-alignment-check.sh" || true
fi
`;
  }
...
`;
  ...
  // Issue grooming (wfl-008)
  if (rules.some(r => r.ruleId === 'wfl-008')) {
    script += `
# Issue grooming and dependency check
if [ -f "${projectRoot}/templates/hooks/issue-grooming-hook.sh" ] && [ "${VIBE_CODEX_GROOM:-true}" = "true" ]; then
  # Run in background to not slow down commits
  (bash "${projectRoot}/templates/hooks/issue-grooming-hook.sh" 2>/dev/null &)
fi
`;
  }
...
async function uninstallGitHooks() {
  try {
    const hooksDir = path.join('.git', 'hooks');
    const hooks = ['pre-commit', 'commit-msg', 'pre-push', 'post-commit', 'post-merge'];
