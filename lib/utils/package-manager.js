...
async function validateSetup() {
  ...
  try {
    ...
    if (major < 14) {
      errors.push(
        `Node.js version ${nodeVersion} is too old. Please upgrade to v14 or higher.`,
      );
    }
  } catch {
  ...
  if (!npxInfo.available) {
    ...
    if (npxInfo.npmVersion) {
      warnings.push(
        `npm ${npxInfo.npmVersion} is installed but npx is not available. Consider upgrading npm.`,
      );
    }
  }
  ...
  if (!pmExists && packageManager !== "npm") {
    warnings.push(
      `Lock file suggests ${packageManager} but it's not installed.`,
    );
  }
