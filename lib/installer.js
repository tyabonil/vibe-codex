...
if [ -f "$QUALITY_SCRIPT" ]; then
  ...
  if (rules.includes('cursor-rules')) {
    checks.push(`
# Cursor rules check
# Check if the cursor rules script exists
CURSOR_SCRIPT="$(git rev-parse --show-toplevel)/templates/hooks/cursor-rules-check.sh"
if [ ! -f "$CURSOR_SCRIPT" ]; then
  # Try node_modules location
  CURSOR_SCRIPT="$(git rev-parse --show-toplevel)/node_modules/vibe-codex/templates/hooks/cursor-rules-check.sh"
fi
