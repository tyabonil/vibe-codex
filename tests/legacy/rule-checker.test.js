...
describe('MANDATORY Rules Compliance Checker', () => {
  ...
  describe('Level 1: Security & Safety', () => {
    describe('Secret Detection', () => {
      ...
      test('should not flag non-secret patterns', async () => {
        const files = [{
          filename: 'config.js',
          patch: '+const config = { api_url: "https://api.example.com" };\n+// TODO: Add API key configuration'
        }];
    ...
    describe('Environment File Protection', () => {
      ...
      test('should allow .env file creation', async () => {
        const files = [{
          ...
          status: 'added',
          patch: '+# Example environment variables\n+API_KEY=your_api_key_here'
        }];
      ...
      test('should detect various environment file patterns', async () => {
        const envFiles = ['.env', '.env.local', '.env.production', 'environment.yml'];
  ...
  describe('Level 2: Workflow Integrity', () => {
    describe('Issue Reference Validation', () => {
      test('should detect missing issue reference', async () => {
        ...
        const violations = await ruleEngine.checkLevel2Workflow(prData, [], [], mockGitHubClient);
        
        const issueViolation = violations.find(v => v.rule === 'CREATE ISSUES FOR ALL WORK');
      ...
      test('should accept valid issue references', async () => {
        ...
        for (const prData of validReferences) {
          ...
          const hasReference = ruleEngine.hasIssueReference(prData.title, prData.body);
    ...
    describe('MCP GitHub API Usage Detection', () => {
      ...
      test('should not flag MCP API usage', () => {
        const commits = [
          { commit: { message: 'Used mcp_github_create_branch for new feature' } },
    ...
    describe('Significant Changes Detection', () => {
      test('should detect significant changes requiring PROJECT_CONTEXT update', () => {
        ...
        const hasSignificant = ruleEngine.hasSignificantChanges(significantFiles);
  ...
  describe('Level 3: Quality Gates', () => {
    ...
    describe('Copilot Review Validation', () => {
      test('should detect draft PRs awaiting Copilot review', async () => {
        ...
        const violation = await ruleEngine.checkCopilotReview(prData, mockGitHubClient);
  ...
  describe('Level 4: Development Patterns', () => {
    describe('File Size Validation', () => {
      test('should flag large files', async () => {
        ...
        const sizeViolation = violations.find(v => v.rule === 'FILES ≤200-300 LINES');
    ...
    describe('Branch Target Validation', () => {
      test('should recommend against merging to main', async () => {
        ...
        const branchViolation = violations.find(v => v.rule === 'NEVER MERGE TO MAIN');
  ...
  describe('Reporter Functionality', () => {
    describe('Report Generation', () => {
      test('should generate comprehensive violation report', () => {
        ...
        const report = reporter.generateReport(violations, 6, true, { number: 123 });
      ...
      test('should generate success report for compliant PRs', () => {
        const report = reporter.generateSuccessReport({ number: 123, title: 'Test PR' });
  ...
  describe('Integration Tests', () => {
    test('should handle complete rule checking workflow', async () => {
      ...
      const files = [
        { filename: 'src/test.js', additions: 50, patch: '+console.log("test");' },
        { filename: 'tests/test.spec.js', additions: 30, patch: '+describe("test")' }
      ];
      ...
      // Test Level 2
      const level2Violations = await ruleEngine.checkLevel2Workflow(prData, files, commits, mockGitHubClient);
      expect(level2Violations.length).toBeLessThan(2); // May have some minor violations

      // Test Level 3
      const level3Violations = await ruleEngine.checkLevel3Quality(prData, files, mockGitHubClient);
      ...
      const allViolations = [...level1Violations, ...level2Violations, ...level3Violations, ...level4Violations];
...
function createMockGitHubClient() {
  return {
    ...
    getPRComments: jest.fn().mockResolvedValue({ issueComments: [], reviewComments: [] }),
