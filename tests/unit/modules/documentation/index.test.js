...
## License
  ...
  test('should detect missing README sections', async () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.warnings).toContain('Missing recommended section: Installation');
  });
...
function complexCalculation(data, options) {
  ...
  return result;
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('Missing JSDoc for function: complexCalculation');
  });
...
function complexAlgorithm(data) {
  ...
  return x.reduce((a, b) => a + b, 0) / x.length;
    ...
    expect(result.warnings).toContain('Complex code without comments in complexAlgorithm');
  });
  ...
  test('should check documentation freshness', async () => {
    ...
    expect(result.warnings).toContain('Documentation may be outdated: README.md');
  });
});